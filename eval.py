...
def process_batch(
    *,
    ...
    colorize_depth = functools.partial(
        viz.colorize, cmin=datasource.near, cmax=datasource.far, invert=True
    )
    ...
    if save_dir:
        ...
        image_utils.save_image(
            save_dir / f"rgb_{item_id}.png", image_utils.image_to_uint8(rgb)
        )
        image_utils.save_image(
            save_dir / f"depth_expected_viz_{item_id}.png",
            image_utils.image_to_uint8(depth_exp_viz),
        )
        image_utils.save_depth(
            save_dir / f"depth_expected_{item_id}.png", depth_med[..., 0]
        )
        image_utils.save_image(
        ...
        image_utils.save_depth(
            save_dir / f"depth_median_{item_id}.png", depth_med[..., 0]
        )
    ...
    if "rgb" in batch:
        ...
        out["psnr"] = psnr
        out["ssim"] = ssim
        logging.info("\tMetrics: mse=%.04f, psnr=%.02f, ssim=%.02f", mse, psnr, ssim)

        rgb_abs_error = viz.colorize(abs(rgb_target - rgb).sum(axis=-1), cmin=0, cmax=1)
    ...
    if "depth" in batch:
        ...
        summary_writer.image(f"depth-target/{tag}/{item_id}", depth_target_viz, step)
        ...
        summary_writer.image(f"relative-disparity/{tag}/{item_id}", rel_disp_pred, step)
    ...
    for i, (item_id, batch) in enumerate(zip(item_ids, iterator)):
        ...
        if tag == "test":
            ...
            if datasource.use_appearance_id:
                ...
                metadata["appearance"] = jnp.full(
                    shape, fill_value=appearance_id, dtype=jnp.uint32
                )
            ...
            if datasource.use_camera_id:
                ...
                metadata["camera"] = jnp.full(
                    shape, fill_value=camera_id, dtype=jnp.uint32
                )
...
def main(argv):
    ...
    gin.parse_config_files_and_bindings(
        config_files=gin_configs, bindings=FLAGS.gin_bindings, skip_unknown=True
    )
    ...
    n_devices = len(devices_to_use) if devices_to_use else jax.local_device_count()
    ...
    val_eval_ids = utils.strided_subset(datasource.val_ids, eval_config.num_val_eval)
