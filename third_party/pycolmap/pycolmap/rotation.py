...
def axis_angle_to_rotation_matrix(axis, angle=None):
    ...
    return np.eye(3) + (
        np.sin(angle) * cp_axis + (1.0 - np.cos(angle)) * cp_axis.dot(cp_axis)
    )
...
class Quaternion:
    ...
    def ToEulerAngles(self):
        ...
        yaw = np.arctan2(
            2.0 * (self.q[0] * self.q[2] - self.q[1] * self.q[3]),
    ...
    def __mul__(self, other):
        if isinstance(other, DualQuaternion):
            return DualQuaternion(
                self.q0 * other.q0, self.q0 * other.qe + self.qe * other.q0
            )
        elif isinstance(other, complex):  # multiplication by a dual number
            return DualQuaternion(
                self.q0 * other.real, self.q0 * other.imag + self.qe * other.real
            )
        else:  # scalar multiplication (assumed)
