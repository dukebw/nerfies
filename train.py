...
def _log_to_tensorboard(
    ...
    writer.scalar("params/elastic_loss/weight", scalar_params.elastic_loss_weight, step)
...
def main(argv):
    ...
    train_dataset = datasource.create_dataset(
        datasource.train_ids, flatten=True, shuffle=True
    )
    ...
    if train_config.use_background_loss:
        points_dataset = (
            ...
            .repeat()
            .shuffle(65536, reshuffle_each_iteration=True, seed=exp_config.random_seed)
        )
    ...
    for step, batch in zip(range(init_step, train_config.max_steps + 1), train_iter):
