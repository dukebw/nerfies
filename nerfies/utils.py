...
def interpolate_codes(
    codes: Union[np.ndarray, List[np.ndarray]], num_samples: int, method="spline"
):
...
def general_loss_with_squared_residual(squared_x, alpha, scale):
    ...
    alpha_safe = jnp.where(
        jnp.greater_equal(alpha, 0.0), jnp.ones_like(alpha), -jnp.ones_like(alpha)
