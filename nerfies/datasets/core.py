...
def load_camera(camera_path, scale_factor=1.0, scene_center=None, scene_scale=None):
...
def camera_to_rays(
    camera: cam.Camera, image_shape: Optional[Tuple[int, int]] = None
) -> Dict[str, np.ndarray]:
    ...
    img_rays_origin = np.tile(camera.position[None, None, :], image_shape + (1,))
...
def dataset_from_dict(data_dict, rng, shuffle=False, flatten=False):
    ...
    logging.info(
        "*** Loaded dataset items: num_rays=%d, num_examples=%d", num_rays, num_examples
    )
...
class DataSource(abc.ABC):
    ...
    def create_cameras_dataset(
        ...
        return dataset_from_dict(
            data_dict, rng=self.rng, flatten=flatten, shuffle=shuffle
        )

    def create_dataset(self, item_ids, flatten=False, shuffle=False) -> tf.data.Dataset:
