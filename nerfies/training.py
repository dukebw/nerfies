...
def save_checkpoint(path, state, keep=100):
    ...
    checkpoint_path = checkpoints.save_checkpoint(path, state_to_save, step, keep=keep)
...
def train_step(
    ...
    def _loss_fn(params):
        ...
        stats = {}
        if "fine" in ret:
            losses["fine"], stats["fine"] = _compute_loss_and_stats(params, ret["fine"])
        if "coarse" in ret:
